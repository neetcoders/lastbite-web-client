...
export async function getPublicProduct ( searchedProduct? : string ){
  try {
    const link = searchedProduct ? `/product/public?limit=20&offset=0&distance=2000000&search=${searchedProduct}` : `/product/public?limit=20&offset=0&distance=2000000`;
    const response = await apiClient.get<ResponseSchema<IProduct[]>>(link);
...
export async function getUserNearestProduct (searchedProduct? : string){
  try {
    const token = await getAuthToken();
    const link = searchedProduct ? `/product?limit=20&offset=0&distance=10000&search=${searchedProduct}` : `/product?limit=20&offset=0&distance=10000`;
    const response = await apiClient.get<ResponseSchema<IProduct[]>>(link, {
      headers: {
        Authorization: `Bearer ${token}`
      }
